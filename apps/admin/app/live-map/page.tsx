...
const LiveLeafletMap = dynamic(() => import('@/components/LiveLeafletMap'), {
  ssr: false,
  loading: () => (
    <div className="flex-1 w-full h-full min-h-[500px] bg-slate-950 flex flex-col items-center justify-center text-slate-400">
...
export default function AdminLiveMapPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col">
      <AdminNavbar />
      {/* Top Bar */}
      <header className="bg-slate-800/90 border-b border-slate-700 px-6 py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 backdrop-blur-xs sticky top-0 z-20">
        <div>
          <div className="flex items-center space-x-3">
            ...
            {isReconnecting ? (
              <span className="flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-900/60 text-amber-300 border border-amber-500/30 animate-pulse">
                <span className="w-2 h-2 rounded-full bg-amber-400" />
                <span>Reconnecting...</span>
              </span>
            ) : realtimeConnected ? (
              <span className="flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-900/60 text-emerald-300 border border-emerald-500/30">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                <span>Realtime GPS Connected</span>
              </span>
            ) : (
              <span className="text-xs text-slate-400">1.5s Live Polling</span>
            )}
          </div>
        ...
        <div className="flex items-center space-x-3">
          <Link
            href="/bookings"
            className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-xs font-bold rounded-xl transition border border-slate-600 shadow-xs"
          >
      ...
      <div className="flex-1 flex flex-col lg:flex-row relative overflow-hidden min-h-[calc(100vh-130px)]">
        ...
        <div className="flex-1 relative flex flex-col min-h-[500px] lg:min-h-[calc(100vh-130px)]">
          <LiveLeafletMap
        ...
        <aside className="w-full lg:w-96 bg-slate-900 border-t lg:border-t-0 lg:border-l border-slate-800 p-5 flex flex-col justify-between overflow-y-auto max-h-screen">
          <div className="space-y-4">
            {/* Category Filter Tabs */}
            <div className="bg-slate-800/80 p-1 rounded-xl border border-slate-700 flex gap-1 text-xs font-semibold">
              <button
                onClick={() => {
            ...
            {selectedDriver ? (
              <div className="space-y-4 pt-1">
                <div className="flex items-center justify-between pb-2.5 border-b border-slate-800">
                  ...
                  <button
                    onClick={() => setSelectedDriver(null)}
                    className="text-slate-400 hover:text-white text-xs bg-slate-800 px-2 py-0.5 rounded-md"
                  >
                    ✕
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-bold text-base text-white">{selectedDriver.user.fullName}</h3>
                    <p className="text-xs text-slate-400 font-mono">{selectedDriver.user.phone}</p>
                  </div>
                  <span
                ...
                {/* Status & Coordinates */}
                <div className="p-3 bg-slate-800/70 rounded-xl border border-slate-700 text-xs space-y-2">
                  ...
                  <div className="pt-1.5 border-t border-slate-700/60 flex items-start justify-between gap-2">
                    <span className="text-slate-400 shrink-0">🏢 Current Area:</span>
                    <div className="text-right">
                      ...
                      {selectedDriver.currentLat && selectedDriver.currentLng && (
                        <a
                          ...
                          className="text-[10px] text-sky-400 hover:text-sky-300 underline font-mono inline-block mt-0.5"
                        >
                          View in Google Maps ↗
                        </a>
                      )}
                  ...
                  <div className="flex justify-between font-mono pt-1 border-t border-slate-700/60">
                  ...
                  <div className="flex justify-between font-mono">
                    ...
                    <span className="text-slate-200">{selectedDriver.licenseNumber}</span>
                  </div>
                  <div className="flex justify-between font-mono">
                    ...
                    {selectedDriver.assignedBookings.map((b) => (
                      <div key={b.id} className="text-xs space-y-1">
                        <div className="flex justify-between">
                          <span className="font-bold text-white">{b.humanReadableRef}</span>
                          <span className="text-indigo-300 font-semibold">{b.status}</span>
                        </div>
                        <p className="text-slate-400 truncate">📍 Pickup: {b.pickupAddress}</p>
                        <p className="text-slate-400 truncate">🏁 Drop: {b.dropAddress}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-[10px] text-indigo-400 font-mono">OpenStreetMap</span>
                  </div>
                  ...
                  {breadcrumbs.length === 0 ? (
                    ...
                    <div className="max-h-40 overflow-y-auto space-y-1 pr-1 font-mono text-[11px]">
                      {breadcrumbs.map((pt, idx) => (
                        <div
                          key={pt.id || idx}
                          className="flex justify-between p-2 rounded-lg bg-slate-800/60 border border-slate-700/50 text-slate-300"
                        >
                          <span>
              ...
              <div className="space-y-2.5">
                <div className="flex justify-between items-center pb-1 border-b border-slate-800">
                  ...
                  <span className="text-[10px] text-indigo-400">Click driver to track</span>
                </div>

                {filteredDrivers.length === 0 ? (
                  <div className="text-center py-12 text-slate-500 text-xs space-y-1">
                    <div className="text-2xl mb-1">🚗</div>
                    <p className="font-semibold text-slate-400">No drivers matching this filter</p>
                    ...
                    {filteredDrivers.map((d) => {
                      ...
                      return (
                        <div
                          ...
                          className="p-3 bg-slate-800/70 hover:bg-slate-800 rounded-xl border border-slate-700 hover:border-indigo-500/60 cursor-pointer transition shadow-xs group"
                        >
                          <div className="flex items-start justify-between gap-2 mb-1.5">
                            <div>
                              <h4 className="font-bold text-sm text-white group-hover:text-indigo-300 transition flex items-center gap-1.5">
                                <span>🚕</span>
                                <span>{d.user.fullName}</span>
                              </h4>
                              <p className="text-[11px] text-slate-400 font-mono">{d.user.phone}</p>
                            </div>
                            <span
                              className={`text-[9px] font-bold px-2 py-0.5 rounded-md border ${
                                isOnTrip
                                  ? isTripStarted
                                    ...
                                    : 'bg-blue-900/50 text-blue-300 border-blue-500/40'
                          ...
                          <div className="text-[11px] text-slate-400 space-y-1 border-t border-slate-700/60 pt-2">
                            <div className="flex justify-between font-mono">
                            ...
                            {resolvedLocation && (
                              <div className="text-[10px] text-emerald-300 flex items-center gap-1 pt-0.5 truncate">
                                <span>🏢</span>
                                <span className="truncate">{resolvedLocation}</span>
                              </div>
                            )}
                            {isOnTrip && (
                              <div className="mt-1 pt-1 border-t border-slate-700/40 text-[10px] text-indigo-300 truncate">
          ...
          <div className="mt-4 pt-3 border-t border-slate-800 flex justify-between text-xs text-slate-400 font-mono">
            <span>Online: <strong className="text-white">{drivers.length}</strong></span>
            <span>Idle: <strong className="text-emerald-400">{idleCount}</strong></span>
            <span>On-Trip: <strong className="text-amber-400">{onTripCount}</strong></span>
          </div>
        </aside>
      </div>
    </div>
