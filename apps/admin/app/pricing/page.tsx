    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-wrap gap-4 items-center">
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Trip Type</label>
            <select
          ...
          <div className="ml-auto text-xs text-slate-400">
            Found <span className="font-bold text-white">{rules.length}</span> active fare rules
          </div>
        </div>
                  ...
                  {rules.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-800/50 transition">
                      <td className="py-3 px-4">
                        ...
                        <span className="inline-block mt-0.5 px-1.5 py-0.2 bg-slate-800 text-amber-300 rounded text-[10px] font-semibold">
                          {r.tripType}
                        </span>
                      </td>

                      <td className="py-3 px-4">
                        <span className="font-bold text-slate-300">{r.fuelType}</span>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            r.isActive ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'
                          }`}
                        >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleSaveRule} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Base Rate / KM (₹)</label>
                  <input
                    type="number"
                    step="0.5"
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Extra KM Rate (₹)</label>
                  <input
                    type="number"
                    step="0.5"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Driver Allowance (₹/day)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Night Charge (₹)</label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Night Start (Hr)</label>
                  <input
                    type="number"
                    min="0"
                    max="23"
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Night End (Hr)</label>
                  <input
                    type="number"
                    min="0"
                    max="23"
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="ruleActive" className="text-slate-300 font-semibold cursor-pointer">
                  Rule Active
                </label>
              </div>
